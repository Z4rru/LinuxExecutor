...
Console::Console() {
    ...
    gtk_text_buffer_create_tag(buffer_, "system", "foreground", "#8b949e", "style", PANGO_STYLE_ITALIC, NULL);
    gtk_text_buffer_create_tag(buffer_, "timestamp", "foreground", "#484f58", "scale", 0.85, NULL);
...
void Console::print(const std::string& msg, Level level) {
    ...
    g_idle_add([](gpointer user_data) -> gboolean {
        // This is a simplified version - in production you'd pair this with the Console instance
...
void Console::append_entry(const Entry& entry) {
    ...
    gtk_text_buffer_insert_with_tags_by_name(buffer_, &end, ts.c_str(), -1, "timestamp", NULL);
    
    // Message
    gtk_text_buffer_get_end_iter(buffer_, &end);
    std::string msg = entry.message + "\n";
    gtk_text_buffer_insert_with_tags_by_name(buffer_, &end, msg.c_str(), -1, 
                                              level_tag(entry.level), NULL);
