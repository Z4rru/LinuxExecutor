...
void Overlay::init(GtkApplication* app) {
    ...
    if (n > 0) {
        ...
        if (monitor) {
            ...
            gtk_window_set_default_size(window_, geom.width, geom.height);
...
void Overlay::shutdown() {
    ...
    initialized_ = false;
    visible_ = false;
    LOG_INFO("Overlay system shut down");
}
...
void Overlay::toggle() {
    if (visible_.load(std::memory_order_acquire)) hide();
    else show();
}
...
void Overlay::render(cairo_t* cr, int width, int height) {
    ...
    std::vector<DrawingObject> visible;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        for (const auto& [id, obj] : objects_) {
            if (obj.visible) visible.push_back(obj);
        }
    }
...
void Overlay::render_line(cairo_t* cr, const DrawingObject& obj) {
    double alpha = 1.0 - obj.transparency;
    if (alpha <= 0) return;
    cairo_set_source_rgba(cr, obj.color_r, obj.color_g, obj.color_b, alpha);
...
void Overlay::render_text(cairo_t* cr, const DrawingObject& obj) {
    ...
    switch (obj.font) {
        case 0: font_face = "Sans"; break;
        case 1: font_face = "Sans"; break;
        case 2: font_face = "IBM Plex Sans"; break;
        case 3: font_face = "Monospace"; break;
    }
    ...
    cairo_text_extents(cr, obj.text.c_str(), &extents);
    ...
    if (obj.outline) {
        cairo_set_source_rgba(cr, obj.outline_r, obj.outline_g,
                              obj.outline_b, alpha);
...
void Overlay::render_square(cairo_t* cr, const DrawingObject& obj) {
    ...
    if (obj.rounding > 0) {
        double r = std::min(obj.rounding, std::min(w, h) / 2.0f);
