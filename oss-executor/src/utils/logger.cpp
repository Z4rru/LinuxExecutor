...
bool Logger::init(const std::string& log_dir) {
    // ── guard: no-op on double init ──
    if (initialized_) {
        return true;
    }

    // ── drop any leftover logger with the same name ──
    // (can happen if shutdown() was called then init() again)
    spdlog::drop("oss");

    // ── always-available console sink ──
    auto console_sink =
    ...
    try {
        ...
        // Not fatal — we still have console logging.
        std::cerr << "[logger] Failed to create log directory '"
                  << log_dir << "': " << e.what() << "\n";
    }

    // ── attempt to create rotating file sink ──
    try {
        constexpr std::size_t max_size  = 5 * 1024 * 1024;  // 5 MiB
