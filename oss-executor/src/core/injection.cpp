    ...
    uintptr_t compile_addr = 0, uintptr_t free_addr = 0)
{
    ...
    e({0x48,0x83,0xEC,0x08});

    e({0x49,0x89,0xFC});      // mov r12, rdi  (save L)
    e({0x48,0x89,0x74,0x24,0x04}); // mov [rsp+4], esi  (save from in stack slot)
    ...
    e({0x48,0x8B,0x74,0x24,0x04}); // mov rsi, [rsp+4]  (restore from)
    e({0x44,0x89,0xF2});      // mov edx, r14d  (restore nargs)
    e({0x48,0x83,0xC4,0x08});
