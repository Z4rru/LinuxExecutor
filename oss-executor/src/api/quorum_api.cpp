...
// ── Safe nested string getter — THE ACTUAL FIX ──────────────────
// Handles: null, missing key, wrong type — never throws
...
// Safe top-level value getter — replaces .value() which throws on null
template<typename T>
...
// ── Parse single script — null-safe version ─────────────────────
static ScriptInfo parse_script(const json& s) {
    ScriptInfo info;
    ...
    // FIX: These two lines were the crash — nested .value() on null
...
// ── api_request — unchanged, already returns json{} on failure ──
...
std::vector<ScriptInfo> QuorumAPI::search_scripts(const std::string& query, int page) {
    ...
    // FIX: Check null AND check nested "result" safely
    ...
    try {
        ...
        // FIX: "scripts" inside "result" could also be null
        ...
        for (const auto& s : result["scripts"]) {
            ...
            ScriptInfo info = parse_script(s);
...
// ── get_trending — FIXED (THIS WAS THE CRASH) ──────────────────
...
// ── get_scripts_for_game — unchanged ────────────────────────────
